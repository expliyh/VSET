  ...
  if (useVfi.value === true) {
    if (VfiMethodValue.value === 'Rife') {
      ...
      if (useFreezeRepair.value === true) {
        ...
        vpyContent += '                continue\n'
        vpyContent += '            if s <= last_end:\n'
        vpyContent += '                continue\n'
        vpyContent += '            clean.append((s, e))\n'
        vpyContent += '            last_end = e\n'
        vpyContent += '        if not clean:\n'
        vpyContent += '            return clip\n'
        vpyContent += '        groups = {}\n'
        vpyContent += '        for (s, e) in clean:\n'
        vpyContent += '            L = e - s + 1\n'
        vpyContent += '            groups.setdefault(L, []).append((s, e))\n'
        vpyContent += '        repaired = {}\n'
        vpyContent += '        for (L, segs) in groups.items():\n'
        vpyContent += '            pair_parts = []\n'
        vpyContent += '            for (s, e) in segs:\n'
        vpyContent += '                a = core.std.Trim(clip, s - 1, s - 1)\n'
        vpyContent += '                b = core.std.Trim(clip, e + 1, e + 1)\n'
        vpyContent += '                pair_parts.append(a)\n'
        vpyContent += '                pair_parts.append(b)\n'
        vpyContent += '            pairs = core.std.Splice(pair_parts, mismatch=True)\n'
        vpyContent += '            pairs = core.std.AssumeFPS(pairs, fpsnum=clip.fps_num, fpsden=clip.fps_den * (L + 1))\n'
        vpyContent += `            v = RIFE(pairs, scale=${RifeScaleValue.value},model=${model},ensemble=${EnsembleBool},multi=Fraction(L + 1, 1), backend=device_vfi)\n`
        vpyContent += '            offsets = list(range(1, L + 1))\n'
        vpyContent += '            between = core.std.SelectEvery(v, cycle=2 * (L + 1), offsets=offsets)\n'
        vpyContent += '            for idx, (s, e) in enumerate(segs):\n'
        vpyContent += '                start = idx * L\n'
        vpyContent += '                repaired[(s, e)] = core.std.Trim(between, start, start + L - 1)\n'
        vpyContent += '        parts = []\n'
        vpyContent += '        cursor = 0\n'
        vpyContent += '        for (s, e) in clean:\n'
        vpyContent += '            if cursor <= s - 1:\n'
        vpyContent += '                parts.append(core.std.Trim(clip, cursor, s - 1))\n'
        vpyContent += '            parts.append(repaired.get((s, e), core.std.Trim(clip, s, e)))\n'
